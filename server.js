...
app.post('/api/submissions', (req, res) => {
  ...
  if (demand_id) {
    db.get(`SELECT unit_price_min, unit_price_max FROM recycle_demands WHERE id=?`, [demand_id], (err, row) => {
      ...
      const stmt = db.prepare(`
        ...
        (demand_id, title, isbn, quantity, condition_grade, expected_price, images, contact,
         status, auto_quote, final_quote, reviewer_id, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, NULL, NULL, ?, ?)
      `);
      stmt.run(
        demand_id, title, isbn || null, Number(quantity), condition_grade || null,
    ...
    `);
    stmt.run(
      title, isbn || null, Number(quantity), condition_grade || null,
      expected_price || null, JSON.stringify(images || []), contact || null,
      now(), now(),
      function (err2) {
        if (err2) return res.status(500).json({ error: err2.message });
